...
namespace Sapper
{
    public partial class Form8 : Form
    {
        string data;
        int BackGround = 1;
        private static string connectionString3 = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=Tabl3.mdb";

        //сама игра 
        public const int mapSize = 25;
        public const int celllSize = 22;
        ...
        int Time;
        static int a, b;
        public Form8(string data, int BacKGround)
        {
            Controls.Clear();
            InitializeComponent();
            Init();
        ...
        private static void OnLeftButtonPressed(Button pressedButton, Form8 form8)
        {
            string data = form8.data;
            int BackGround = form8.BackGround;
            ...
            if (revealedCellsCount == mapSize * mapSize - number)  // number is the total number of bombs
            {
                ...
                using (OleDbConnection dbConnection1 = new OleDbConnection(connectionString3))
                {
                    try
                    {
                        dbConnection1.Open();
                        string query1 = "SELECT COUNT(*) FROM Tabl3 WHERE Login = @Login";
                        using (OleDbCommand command1 = new OleDbCommand(query1, dbConnection1))
                        {
                            ...
                            if (count > 0)
                            {
                                string query4 = "SELECT Time FROM Tabl3 WHERE Login = @data";
                                int count2 = 0;
                                ...
                                if (count2 > Time)
                                {
                                    string query2 = "UPDATE [Tabl3] SET [Time] = @Time WHERE [Login] = @Login";
                            ...
                            {
                                using (OleDbConnection dbConnection3 = new OleDbConnection(connectionString3))
                                {
                                    dbConnection3.Open();
                                    string query3 = "INSERT INTO [Tabl3] VALUES (@Login, @Time)";
